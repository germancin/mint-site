...
export const loadContract = async (dispatch, web3, netId) => {
	try {
		...
		console.log("CONTRACT:", web3, netId)

		const contract = new web3.eth.Contract(Contract.abi, Contract.networks[netId].address)

		console.log("::::CONTRACT::::", Contract.abi, Contract.networks)

		dispatch(contractLoaded(contract))
		return contract
	} catch (e) {
		window.alert('Wrong network!', e.message)
		console.log('Error, load contract: ', e.message)
...
export const update = async (dispatch) => {
	try{
		...
		console.log("DONE TO UPDATE account:::", account)
		console.log("DONE TO UPDATE web3:::", web3)
		console.log("DONE TO UPDATE netId:::", netId)
		console.log("DONE TO UPDATE contract:::", contract)

		if(account && contract){
			console.log("THERE IS ACCOUNT AND CONTRACT", contract)
...
export const loadNftData = async (dispatch, contract) => {
	try{
		...
		console.log("URI CALLED TO GET SUPPLYES METADATA:::::", uri);
		fetch(uri)
			.then(res => {
			...
			.then(result => {
				...
				console.log("TEHN RESULTS", result)
		...
		console.log("TODAO nftsData:::", nftsData)
	} catch (e) {
...
export const loadNftState = async (dispatch, contract) => {
	try{
		...
		console.log("VAS FOR TOTAL SUPPLY", totalSupply)
